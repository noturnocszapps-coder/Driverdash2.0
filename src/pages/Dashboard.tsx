...
import { startOfDay, isSameDay, parseISO, subDays, format, differenceInMinutes } from 'date-fns';
...
export const Dashboard = () => {
  ...
  const openCycle = useMemo(() => {
    try {
      ...
      console.error('[Dashboard] openCycle error:', error);
      return null;
    }
  }, [cycles]);
  ...
  const currentVehicle = useMemo(() => {
    try {
      const profiles = settings?.vehicleProfiles || [];
      return profiles.find((v: any) => v?.id === settings?.currentVehicleProfileId) || null;
    } catch (error) {
      console.error('[Dashboard] currentVehicle error:', error);
      return null;
    }
  ...
  const profitStats = useMemo(() => {
    try {
      ...
      console.error('[Dashboard] profitStats error:', error);
      return null;
    }
  ...
  const cycleProgress = useMemo(() => {
    try {
      ...
      const start = safeDate(openCycle?.start_time);
      const total = 24 * 60;
      ...
      console.error('[Dashboard] cycleProgress error:', error);
      return null;
    }
  }, [openCycle, now]);
  ...
  const stats = useMemo(() => {
    try {
      ...
      const chartData = last7Days.map((date) => {
        const dayCycles = (cycles || []).filter((c: any) => {
          try {
            return c?.start_time && isSameDay(parseISO(c.start_time), date);
          } catch {
            return false;
          }
        });

        const total = dayCycles.reduce((acc: number, c: any) => acc + safeNumber(c?.total_amount), 0);
      ...
      const total7Days = chartData.reduce((acc, d) => acc + safeNumber(d?.value), 0);
      const avg = total7Days / 7;

      return { chartData, total7Days, avg };
    } catch (error) {
      console.error('[Dashboard] stats error:', error);
      return { chartData: [], total7Days: 0, avg: 0 };
    }
  }, [cycles]);
  ...
  const firstName = (settings?.name || 'Motorista').split(' ')[0];
        ...
        <CardContent className="p-8 space-y-8 relative z-10">
          <div className="flex justify-between items-start gap-4">
            <div className="space-y-1 min-w-0">
              ...
              <h2 className="text-4xl md:text-5xl font-black tracking-tighter break-words">
                ...
                <div className="flex justify-between items-end">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider">
                      Iniciado às {format(safeDate(openCycle?.start_time), 'HH:mm')}
                    </span>
                  </div>
            ...
            <CardContent className="p-0">
              <div className="p-5 border-b border-zinc-50 dark:border-zinc-800/50 flex justify-between items-center">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-black">{formatKm(safeNumber(openCycle?.total_km))}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 divide-x divide-zinc-50 dark:divide-zinc-800/50">
                <div className="p-4 space-y-1">
                  ...
                  <p className="text-sm font-black tracking-tight">{formatKm(safeNumber(openCycle?.ride_km))}</p>
                </div>
                <div className="p-4 space-y-1">
                  ...
                  <p className="text-sm font-black tracking-tight text-zinc-500">
                    {formatKm(safeNumber(openCycle?.displacement_km))}
                  </p>
                </div>
                <div className="p-4 space-y-1">
                    ...
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-zinc-900 text-white px-3 py-2 rounded-xl text-[10px] font-black shadow-2xl border border-white/5">
                          {formatCurrency(safeNumber(payload[0]?.value))}
                        </div>
                      );
                    }
                    return null;
